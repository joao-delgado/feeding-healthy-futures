...
```dataviewjs
const notes = dv.pages().filter(p => p.file.path.includes("1_Current Reality and Urgencies/")); 
...
```dataviewjs
const pagesInFolder = dv.pages().filter(p => p.file.path.includes("1_Current Reality and Urgencies/")).array();
...
--- start-multi-column: Dashboard_Current_Reality_ and_Urgencies_2
```column-settings  
Border: disabled  
Shadow: off
```
...
```dataviewjs
const folderPath = "Feeding Healthy Futures/1_Current Reality and Urgencies";
...
#- chart data    -#
#-----------------#
data: "wordcount:1_Current Reality and Urgencies/"
