...
```dataviewjs
const notes = dv.pages().filter(p => p.file.path.includes("4_Balanced Future Filter/")); 
...
```dataviewjs
const pagesInFolder = dv.pages().filter(p => p.file.path.includes("4_Balanced Future Filter/")).array();
...
```dataviewjs
const folderPath = "Feeding Healthy Futures/4_Balanced Future Filter";
...
#- chart data    -#
#-----------------#
data: "wordcount:4_Balanced Future Filter"
