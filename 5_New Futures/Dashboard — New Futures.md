...
```dataviewjs
const notes = dv.pages().filter(p => p.file.path.includes("5_New Futures/")); 
...
```dataviewjs
const pagesInFolder = dv.pages().filter(p => p.file.path.includes("5_New Futures/")).array();
...
```dataviewjs
const folderPath = "Feeding Healthy Futures/5_New Futures";
