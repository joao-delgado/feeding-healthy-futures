...
--- start-multi-column: Dashboard_Analyse_Future_Predictions
```column-settings 
number of columns: 3
Border: disabled  
Shadow: off
...
```dataviewjs
const notes = dv.pages().filter(p => p.file.path.includes("2_Analyse Future Predictions/")); 
...
```dataviewjs
const pagesInFolder = dv.pages().filter(p => p.file.path.includes("2_Analyse Future Predictions/")).array();
...
```dataviewjs
const folderPath = "Feeding Healthy Futures/2_Analyse Future Predictions";
...
#- chart data    -#
#-----------------#
data: "wordcount:2_Analyse Future Predictions"
